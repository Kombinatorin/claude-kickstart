...
1. **Create the capture file** at `operations/brainstorms/{YYYY-MM-DD}-{topic-slug}.md` (create the `operations/brainstorms/` folder if it doesn't exist). Every brainstorm capture lives here. One predictable home, regardless of topic. Do NOT scatter captures into project folders. If a session later produces a polished deliverable (a plan, a map, a spec), that artifact can move into the relevant `projects/active/` folder, but the raw capture always stays in `operations/brainstorms/`.
...
- If a question can be answered by **exploring the codebase or reading a file/doc**, do that instead of asking. Read `context/me.md`, `context/work.md`, `context/goals.md`, `context/team.md`, `context/preferences.md`, `context/tools-stack.md`, `context/current-priorities.md` first if the topic touches those areas. If the user hands you a doc (e.g. a Google Doc), read it and only surface what's net-new.
...
- Suggest **promotion paths** if applicable:
  - Decisions → `decisions/log.md` (use `templates/decision-entry.md`)
  - Concrete tasks → `operations/inbox.md`
  - New initiative → `projects/active/<x>/STRATEGY.md`
- The brainstorm file itself stays in `operations/brainstorms/` as the raw audit trail. Never delete or move it.

---

**Attribution**: Original interview pattern distributed via Nate Herk. Local adaptation for AI.Instructor: capture path moved from `brainstorms/` to `operations/brainstorms/`; added context-file pre-read hint and promotion-path recap.
